...
type SortField = 'brand' | 'retailerCount' | 'avgPrice' | 'views';
...
interface AggregateByBrandsTabProps {
...
export default function AggregateByBrandsTab({ retailers, selectedBrandName, selectedBrandsHeader, showKPIs = true, showFilters = true }: AggregateByBrandsTabProps) {
  ...
  const allBrandNames = useMemo(() => {
    const fromSkus = new Set(allSkus.map(s => s.brand));
    if (selectedBrandsHeader && selectedBrandsHeader.length > 0 && !selectedBrandsHeader.includes('All Brands')) {
      for (const b of selectedBrandsHeader) fromSkus.add(b);
    }
    return Array.from(fromSkus);
  }, [allSkus, selectedBrandsHeader]);

  const aggregatedRows: AggregatedRow[] = useMemo(() => {
    const byBrand: Record<string, { retailerSet: Set<string>; priceSum: number; views: number; skuCount: number }> = {};
    for (const s of filteredSkus) {
      if (!byBrand[s.brand]) byBrand[s.brand] = { retailerSet: new Set(), priceSum: 0, views: 0, skuCount: 0 };
      const rec = byBrand[s.brand];
      rec.retailerSet.add(s.retailer);
      rec.priceSum += s.price;
      rec.views += s.views;
      rec.skuCount += 1;
    }
    return allBrandNames.map((brand) => {
      const rec = byBrand[brand];
      if (!rec) return { brand, skuCount: 0, retailerCount: 0, avgPrice: 0, views: 0 };
      return {
        brand,
        skuCount: rec.skuCount,
        retailerCount: rec.retailerSet.size,
        avgPrice: rec.skuCount ? rec.priceSum / rec.skuCount : 0,
        views: rec.views,
      };
    });
  }, [filteredSkus, allBrandNames]);
    ...
    return [...aggregatedRows].sort((a, b) => {
      ...
      switch (sortField) {
        case 'brand':
          ...
          break;
        case 'retailerCount':
          aVal = a.retailerCount;
          bVal = b.retailerCount;
          break;
        case 'avgPrice':
          aVal = a.avgPrice;
          bVal = b.avgPrice;
  ...
  const numBrands = aggregatedRows.length;
  const leaderBrandByViews = useMemo(() => {
    let best: AggregatedRow | null = null;
    for (const r of aggregatedRows) if (!best || r.views > best.views) best = r;
    return best?.brand || '-';
  }, [aggregatedRows]);
  const leaderBrandBySkus = useMemo(() => {
    let best: AggregatedRow | null = null;
    for (const r of aggregatedRows) if (!best || r.skuCount > best.skuCount) best = r;
    return best?.brand || '-';
  }, [aggregatedRows]);
  const avgBrandViews = aggregatedRows.length ? aggregatedRows.reduce((s, r) => s + r.views, 0) / aggregatedRows.length : 0;
    ...
    <div className="w-full">
      ...
      {showKPIs && (
        ...
        <div className="flex-1 bg-[#F5F8FF] p-4 rounded relative flex flex-col items-center justify-center group">
          ...
          <div className="flex items-center gap-1">
            <div className="text-[14px] leading-[20px] tracking-[0.3px] text-[#092540]">Number of Brands</div>
            <InformationCircleIcon className="w-4 h-4 text-[#B6BEC6]" />
          </div>
          <div className="text-[20px] leading-[24px] text-[#092540] font-medium tracking-[1px]">{numBrands}</div>
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-[#092540] text-white text-[12px] rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20">
            Count of brands in filtered results
          </div>
        </div>
        <div className="flex-1 bg-[#F5F8FF] p-4 rounded relative flex flex-col items-center justify-center group">
          <div className="absolute inset-0 rounded border border-[#E6E9EC] pointer-events-none" />
          <div className="flex items-center gap-1">
            <div className="text-[14px] leading-[20px] tracking-[0.3px] text-[#092540]">Leader Brand by Views</div>
            <InformationCircleIcon className="w-4 h-4 text-[#B6BEC6]" />
          </div>
          <div className="text-[20px] leading-[24px] text-[#092540] font-medium tracking-[1px]">{leaderBrandByViews}</div>
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-[#092540] text-white text-[12px] rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20">
            Brand with the highest total views in filtered results
          </div>
        </div>
        <div className="flex-1 bg-[#F5F8FF] p-4 rounded relative flex flex-col items-center justify-center group">
          <div className="absolute inset-0 rounded border border-[#E6E9EC] pointer-events-none" />
          <div className="flex items-center gap-1">
            <div className="text-[14px] leading-[20px] tracking-[0.3px] text-[#092540]">Leader Brand by SKUs</div>
            <InformationCircleIcon className="w-4 h-4 text-[#B6BEC6]" />
          </div>
          <div className="text-[20px] leading-[24px] text-[#092540] font-medium tracking-[1px]">{leaderBrandBySkus}</div>
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-[#092540] text-white text-[12px] rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20">
            Brand with the highest number of SKUs
          </div>
        </div>
        <div className="flex-1 bg-[#F5F8FF] p-4 rounded relative flex flex-col items-center justify-center group">
          <div className="absolute inset-0 rounded border border-[#E6E9EC] pointer-events-none" />
          <div className="flex items-center gap-1">
            <div className="text-[14px] leading-[20px] tracking-[0.3px] text-[#092540]">Average Brand Views</div>
            <InformationCircleIcon className="w-4 h-4 text-[#B6BEC6]" />
          </div>
          <div className="text-[20px] leading-[24px] text-[#092540] font-medium tracking-[1px]">{abbr(Math.round(avgBrandViews))}</div>
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-[#092540] text-white text-[12px] rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20">
      ...
      <div className="w-full">
        <div className="grid grid-cols-[minmax(280px,1.2fr)_0.6fr_0.6fr_0.6fr] text-[12px] text-[#3a5166] border-b border-[#E6E9EC] h-12 items-center" style={{ fontFamily: 'Roboto, sans-serif' }}>
          ...
          >
            Brand Name {getSortIcon('brand')}
          </button>
          <button 
            className="flex items-center gap-1 hover:text-[#195AFE] cursor-pointer justify-start px-4 h-full" 
            onClick={() => handleSort('retailerCount')}
          >
            Number of Retailers {getSortIcon('retailerCount')}
          </button>
          <button 
            className="flex items-center gap-1 hover:text-[#195AFE] cursor-pointer justify-start px-4 h-full" 
        ...
        {rows.map(r => (
          <div key={r.brand} className="grid grid-cols-[minmax(280px,1.2fr)_0.6fr_0.6fr_0.6fr] text-[14px] text-[#092540] h-20 items-center border-b border-[#E6E9EC]">
            <div className="flex items-center gap-3 min-w-0 border-right px-4 h-full border-r border-[#E6E9EC]">
              <div className="w-12 h-12 bg-white rounded-md border border-[#E6E9EC] overflow-hidden">
                <img 
                  src={`https://picsum.photos/seed/${encodeURIComponent(r.brand)}/48/48`}
                  alt={r.brand}
            ...
            <div className="px-4 h-full flex items-center">{r.retailerCount.toLocaleString()}</div>
